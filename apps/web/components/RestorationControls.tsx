    ...
    <div className="space-y-5">
      <div>
        <div className="mb-3 flex items-center justify-between gap-4">
          ...
          <p className="text-xs text-heritage-paper/52">Authenticity first</p>
        </div>
          ...
          {MODES.map((item) => (
            <button
              className={cx(
                ...
                mode === item.value
                  ? "border-heritage-gold/70 bg-heritage-gold/14"
                  : "border-white/10 bg-white/[0.045] hover:border-heritage-gold/35",
              )}
              disabled={disabled}
              key={item.value}
            ...
            >
              ...
              <span className="mt-2 block text-xs leading-5 text-heritage-paper/58">{item.description}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <label className="flex min-h-16 items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.045] px-4">
        <span>
          ...
          <span className="block text-xs text-heritage-paper/58">Uses Gemini only when your backend is configured.</span>
        </span>
        <input
          checked={useAi}
