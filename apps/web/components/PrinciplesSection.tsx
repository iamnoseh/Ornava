...
export function PrinciplesSection({ copy }: PrinciplesSectionProps) {
  return (
    <section className="section-shell py-20" id="principles">
      <div className="heritage-band rounded-lg p-7 sm:p-10">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {copy.items.map((item) => (
            <div className="rounded-lg border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-surface)_70%,transparent)] px-4 py-5 text-sm font-semibold text-[var(--color-text)]" key={item}>
              {item}
            </div>
          ))}
        </div>
