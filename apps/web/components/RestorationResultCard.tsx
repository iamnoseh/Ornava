  ...
  return (
    <aside className="rounded-[24px] border border-white/10 bg-white/[0.055] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-heritage-gold/78">Restoration record</p>
      ...
      {result.provider_error_code && (
        <div className="mt-5 rounded-2xl border border-heritage-gold/24 bg-heritage-gold/10 p-4">
          <p className="text-sm font-semibold text-heritage-paper">
            {isQuota ? "Gemini limit reached" : "Provider fallback"}
          </p>
          <p className="mt-2 text-sm leading-6 text-heritage-paper/68">
      ...
      <p className="mt-5 text-sm leading-6 text-heritage-paper/62">{result.message}</p>
    </aside>
  );
}
...
function Meta({ label, value, tone = "normal" }: { label: string; value: string; tone?: "normal" | "warn" }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-heritage-black/22 p-4">
      <dt className="text-xs uppercase tracking-[0.18em] text-heritage-paper/42">{label}</dt>
